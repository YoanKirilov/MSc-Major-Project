...
DEEP_UDP_PORTS = (
    53, 67, 68, 69, 123, 137, 138, 161, 162, 500, 514, 1194, 1434, 1701,
    1812, 1813, 1900, 2049, 3478, 4500, 5060, 5353, 5683, 11211, 51820,
)
...
LIGHT_SAFE_SCRIPTS = (
    "dns-recursion,snmp-info,upnp-info,http-title,http-headers,ssl-cert"
)
DEEP_SAFE_SCRIPTS = (
...
def _port_selection(
    tcp_ports: tuple[int, ...], udp_ports: tuple[int, ...]
) -> str:
    return f"T:{','.join(str(port) for port in tcp_ports)},U:{','.join(str(port) for port in udp_ports)}"
...
def host_command(
    ...
    if profile == DEEP_PROFILE:
        return [
            nmap_path, "-n", "-Pn", "-sT", "-sU", "-sV", "--version-all",
            *interface_args,
            "--reason", "--script", DEEP_SAFE_SCRIPTS, "--script-timeout", "30s",
            "--max-retries", "2", "--host-timeout", "900s",
            "-p", _deep_port_selection(), "-oX", "-", ip,
        ]
    return [
        nmap_path,
        "-n",
