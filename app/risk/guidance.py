...
GUIDANCE_FIELDS = (
    "title", "rule_version", "limitations", "fixed_explanation", "actions", "references",
)
...
def guidance_updates(document: ScanDocument) -> dict:
    ...
    for device in document.devices:
        services = [service for service in document.services if service.device_id == device.device_id]
    ...
    for saved in document.findings:
        ...
        if latest is None or (latest.severity, latest.confidence) != (saved.severity, saved.confidence):
            continue
...
def refresh_guidance(document: ScanDocument) -> ScanDocument:
    if document.state not in {"completed", "partial", "failed", "cancelled"} or document.phase != "finished":
    ...
    findings = [
        finding.model_copy(update={
            key: getattr(updates[finding.finding_id], key) for key in GUIDANCE_FIELDS
        }) if finding.finding_id in updates else finding
        for finding in document.findings
    ]
    versions = dict(document.versions)
    if all(f.rule_id in RULE_CATALOGUE and f.rule_version == RULE_CATALOGUE[f.rule_id]["version"] for f in findings):
