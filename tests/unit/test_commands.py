...
def test_commands_are_fixed_argument_lists():
    ...
    light_ports = (
        f"T:{','.join(str(port) for port in PORTS)},U:{','.join(str(port) for port in UDP_PORTS)}"
    )
    ...
    discovery_with_interface = discovery_command("/usr/bin/nmap", "192.168.56.0/24", "eth0")
    assert discovery_with_interface[-5:] == ["-e", "eth0", "-oX", "-", "192.168.56.0/24"]
    host_with_interface = host_command("/usr/bin/nmap", "192.168.56.10", interface="eth0")
