...
def test_live_scan_reports_missing_nmap_without_creating_fake_result(tmp_path, monkeypatch):
    ...
    assert not list((tmp_path / "scans").glob("*/scan.json")) if (tmp_path / "scans").exists() else True
...
def test_deep_scan_rejects_multiple_hosts_before_creating_a_job(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(session_manager=manager), base_url=BASE_URL) as client:
        headers = authenticate_client(client, manager)
        response = client.post("/api/live-scans", json={
            "mode": "known_hosts", "profile": "deep-tcp-v1",
            "hosts": ["192.168.56.1", "192.168.56.2"], "authorised": True,
        }, headers=headers)
    assert response.status_code == 422
...
def test_saved_report_ai_endpoint_requires_opt_in_and_does_not_rescan(tmp_path, monkeypatch):
    monkeypatch.setenv("APP_DATA_DIR", str(tmp_path))
    manager = SessionManager()
    with TestClient(create_app(session_manager=manager), base_url=BASE_URL) as client:
        headers = authenticate_client(client, manager)
        ...
        disabled = client.post(f"/api/live-scans/{scan.scan_id}/explanations", json={}, headers=headers)
        assert disabled.status_code == 409
        assert "Settings" in disabled.json()["detail"]
        ...
        accepted = client.post(f"/api/live-scans/{scan.scan_id}/explanations", json={}, headers=headers)
        assert accepted.status_code == 202
        assert called == [scan.scan_id]
        assert client.app.state.store._load_scan(scan.scan_id).findings == [finding]


def test_refresh_guidance_requires_csrf_preserves_evidence_and_does_not_rescan(tmp_path, monkeypatch):
    monkeypatch.setenv("APP_DATA_DIR", str(tmp_path))
    manager = SessionManager()
    ...
    with TestClient(create_app(session_manager=manager), base_url=BASE_URL) as client:
        ...
        assert client.post(url + "/refresh-guidance", json={"expected_revision": 1}, headers={"Origin": BASE_URL}).status_code == 403
        assert client.post(url + "/refresh-guidance", json={"expected_revision": 99}, headers=headers).status_code == 409
        assert client.post(url + "/refresh-guidance", json={"expected_revision": 1}, headers=headers).status_code == 200
        result = client.get(url).json()
        assert not result["guidance_status"]["refresh_available"]
        assert result["guidance_history"][0]["findings"] == [finding.model_dump(mode="json") for finding in scan.findings]
        assert result["findings"][0]["evidence"] == scan.findings[0].model_dump(mode="json")["evidence"]
        assert result["devices"] == [device.model_dump(mode="json") for device in scan.devices]
        assert result["ai_requests_used"] == 0
        client.app.state.store._update_scan(scan.scan_id, lambda current: current.model_copy(update={"phase": "analysis"}), None)
        assert client.post(url + "/refresh-guidance", json={"expected_revision": 3}, headers=headers).status_code == 409
